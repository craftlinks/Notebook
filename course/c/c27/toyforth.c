...
typedef void (*Callback)(tf_ctx *ctx, tfo *name);
...
void *xrealloc(void *ptr, size_t size) {
    ...
    if (!new_ptr) {
        fprintf(stderr, "Out of memory reallocating %zu bytes failed\n", size);
        exit(EXIT_FAILURE);
    }
    return new_ptr;
}
...
/* Basic Library Functions */
 void basic_math_functions(tf_ctx *ctx, tfo *name) {
     if (!ctx_check_stack_min_length(ctx,2)) {
         return;
     }
     tfo *b = ctx_stack_pop(ctx, TFO_TYPE_INT);
     tfo *a = ctx_stack_pop(ctx, TFO_TYPE_INT);
     if (!a || !b) {
         return;
     }
     ...
     switch (name->str.s[0]) {
         case '+':
             result = a->i + b->i;
             ctx_stack_push(ctx, TFO_TYPE_INT, create_tfo_int(result));
             break;
         case '-':
             result = a->i - b->i;
             ctx_stack_push(ctx, TFO_TYPE_INT, create_tfo_int(result));
...
int compare_tfo_string(tfo *a, tfo *b) {
    size_t min_len = a->str.len < b->str.len ? a->str.len : b->str.len;
    int cmp = memcmp(a->str.s, b->str.s, min_len);
    if (cmp == 0) {
...
FunctionTableEntry *get_function_by_name(tf_ctx *ctx, tfo *name) {

    for (size_t i = 0; i < ctx->func_table.func_count; i++) {
        FunctionTableEntry *fe = ctx->func_table.func_table[i];
        if (compare_tfo_string(fe->name, name) == 0) {
            return fe;
        }
    }
    return NULL;
...
FunctionTableEntry *register_user_function(tf_ctx *ctx, tfo *name) {
    ctx->func_table.func_table = xrealloc(ctx->func_table.func_table, sizeof(FunctionTableEntry*) * (ctx->func_table.func_count + 1));
    FunctionTableEntry *fe = xmalloc(sizeof(FunctionTableEntry));
    ctx->func_table.func_table[ctx->func_table.func_count] = fe;
    ctx->func_table.func_count++;
    fe->name = name;
    retain(name);
    fe->func = NULL;
    fe->user_func = NULL;
...
void register_c_function(tf_ctx *ctx, char *name, Callback function) {
    FunctionTableEntry *fe;
    tfo *oname = create_tfo_string(name, strlen(name));
    ...
    if (fe) {
        ...
        fe = register_user_function(ctx, oname);
        fe->func = function;
    }
    release(oname);
}
...
int call_symbol(tf_ctx *ctx, tfo *symbol) {
    ...
    FunctionTableEntry *fe =get_function_by_name(ctx, symbol->str.s);
    if (!fe) {
